...
const CommunityPage = () => {
  ...
  const Board = ({
    ...
    board: { id: number; title: string; nickname: string; product: string; lastChat: string; buyers: number };
  }) => {
    const content = [
    ...
    const onClick = () => navigate(`/community/${board.id}`);
    ...
    return (
      ...
      >
        <header className="relative flex items-center justify-center w-full ">
          <h3 className="flex-1 text-xl font-bold text-custom-green">{board.title}</h3>
          <div className="absolute right-0 text-custom-gray-dark">참여 인원: {board.buyers}</div>
        </header>
